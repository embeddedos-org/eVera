...
class LLMSettings(BaseSettings):
    ...
    lm_studio_url: str = Field("http://localhost:1234", description="LM Studio API URL")
    lm_studio_model: str = Field("auto", description="LM Studio model (auto = use loaded model)")
    # Jan AI — OpenAI-compatible local server (default port 1337)
    jan_url: str = Field("http://localhost:1337", description="Jan AI API URL")
    jan_model: str = Field("auto", description="Jan AI model (auto = use loaded model)")
    # llama.cpp server — OpenAI-compatible (default port 8080)
    llamacpp_url: str = Field("http://localhost:8080", description="llama.cpp server API URL")
    llamacpp_model: str = Field("auto", description="llama.cpp model (auto = use loaded model)")
