...
Drop a Python file in the `plugins/` directory and Voca auto-loads it.
...
class PluginManager:
    ...
    def _load_plugin(self, name: str) -> None:
        ...
        spec = importlib.util.spec_from_file_location(
            f"voca_plugin_{name}", str(plugin_path),
        )
        if spec is None or spec.loader is None:
            raise ImportError(f"Cannot load plugin: {name}")

        module = importlib.util.module_from_spec(spec)
        sys.modules[f"voca_plugin_{name}"] = module
    ...
    def _create_example_plugin(self) -> None:
        ...
        example.write_text('''"""Example Voca plugin -- rename this file (remove underscore) to activate."""
