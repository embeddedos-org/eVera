...
ALPACA_API_KEY = os.getenv("VOCA_ALPACA_API_KEY", "")
ALPACA_SECRET_KEY = os.getenv("VOCA_ALPACA_SECRET_KEY", "")
ALPACA_PAPER = os.getenv("VOCA_ALPACA_PAPER", "true").lower() == "true"

IBKR_HOST = os.getenv("VOCA_IBKR_HOST", "127.0.0.1")
IBKR_PORT = int(os.getenv("VOCA_IBKR_PORT", "7497"))  # 7497=paper, 7496=live
IBKR_CLIENT_ID = int(os.getenv("VOCA_IBKR_CLIENT_ID", "1"))

# Safety: auto-trade threshold (in dollars). Orders above this need confirmation.
AUTO_TRADE_LIMIT = float(os.getenv("VOCA_AUTO_TRADE_LIMIT", "500"))
...
class AlpacaTradeTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> dict[str, Any]:
        if not ALPACA_API_KEY:
            return {"status": "error", "message": "Alpaca not configured. Set VOCA_ALPACA_API_KEY and VOCA_ALPACA_SECRET_KEY in .env"}
...
class TradingViewWebhookTool(Tool):
    """Set up TradingView webhook alerts that trigger Voca trades."""

    def __init__(self) -> None:
        super().__init__(
            name="tradingview_setup",
            description="Configure TradingView webhook alerts to trigger auto-trades via Voca",
            parameters={
        ...
        if action == "setup":
            return {
                ...
                "instructions": (
                    ...
                    "5. Voca will auto-execute the trade when the alert fires!"
                ),
            }
        else:
...
class SmartTradeTool(Tool):
    ...
    def __init__(self) -> None:
        ...
        from voca.brain.agents.income import PaperTradeTool
    ...
    async def execute(self, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            from voca.brain.agents.income import GetStockPriceTool
        ...
        result["note"] = "No broker configured — executed as paper trade. Set VOCA_ALPACA_API_KEY for real trading."
        return result
