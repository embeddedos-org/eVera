"""Email Manager — full IMAP/SMTP email management for eVoca.

@file voca/brain/agents/email_manager.py
...
Requires VOCA_IMAP_* and VOCA_SMTP_* env vars configured.
"""
...
def _get_imap_connection():
    host = os.getenv("VOCA_IMAP_HOST", "")
    port = int(os.getenv("VOCA_IMAP_PORT", "993"))
    user = os.getenv("VOCA_IMAP_USER", "") or os.getenv("VOCA_SMTP_USER", "")
    pwd = os.getenv("VOCA_IMAP_PASS", "") or os.getenv("VOCA_SMTP_PASS", "")

    if not host or not user:
        return None, "IMAP not configured. Set VOCA_IMAP_HOST, VOCA_IMAP_USER, VOCA_IMAP_PASS in .env"
...
class ReplyEmailTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if not smtp_host or not smtp_user:
            return {"status": "error", "message": "SMTP not configured. Set VOCA_SMTP_HOST, VOCA_SMTP_USER, VOCA_SMTP_PASS"}
