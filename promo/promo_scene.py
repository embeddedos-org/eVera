"""eVera — production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eVera", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Formal Verification Framework", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "Python, Z3 SMT, TLA+".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "Model Checking Engine", "Exhaustive state-space exploration verifies temporal logic properties", DUR["f1"]),
            ("02", "Property-Based Testing", "Generates thousands of randomized test cases targeting edge conditions", DUR["f2"]),
            ("03", "Proof Automation", "SMT solver integration automates theorem proving for critical invariants", DUR["f3"]),
        ]
        ...
        components = ["Parser", "State Explorer", "SAT Solver", "Proof Engine", "Reporter"]
        boxes = VGroup()
