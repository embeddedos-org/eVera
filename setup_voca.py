#!/usr/bin/env python3
"""Voca Setup Script — installs all dependencies and configures the environment.

Usage:
    python setup_voca.py          # Full install
    python setup_voca.py --minimal  # Core only (no browser/trading)
    python setup_voca.py --check    # Check what's installed
"""
...
def configure_system_for_247():
    ...
    if system == "Windows":
        ...
        startup_script = Path("start_voca.bat")
        ...
        startup_script.write_text(
            f'@echo off\n'
            f'cd /d "{work_dir}"\n'
            f'echo Starting Voca AI Buddy...\n'
            f'start "" "http://localhost:{port}"\n'
            f'timeout /t 3 >nul\n'
            f'"{venv_python}" "{main_py}" --mode server\n',
            encoding="utf-8",
        )
        print("  ✅ Created start_voca.bat (auto-opens browser)")
  ...
  <Actions>
    <Exec>
      ...
      <WorkingDirectory>{work_dir}</WorkingDirectory>
        ...
        task_file = Path("voca_task.xml")
        task_file.write_text(task_xml, encoding="utf-16")
        result = run(f'schtasks /create /tn "VocaServer" /xml "{task_file}" /f')
        if result and result.returncode == 0:
            print("  ✅ Added to Task Scheduler — Voca starts on boot and auto-restarts on crash")
        else:
...
"""
        service_path = Path("/etc/systemd/system/voca.service")
        try:
            service_path.write_text(service)
            run("sudo systemctl daemon-reload")
            run("sudo systemctl enable voca")
            run("sudo systemctl start voca")
            print("  ✅ Created systemd service — Voca runs 24/7 and auto-restarts")
        except PermissionError:
            print("  ⚠ Need sudo. Run: sudo cp voca.service /etc/systemd/system/")
            Path("voca.service").write_text(service)
...
<plist version="1.0">
<dict>
    <key>Label</key><string>com.voca.server</string>
    ...
    <key>KeepAlive</key><true/>
        ...
        plist_path = Path.home() / "Library/LaunchAgents/com.voca.server.plist"
        plist_path.write_text(plist)
        run(f"launchctl load {plist_path}")
        print("  ✅ Created launchd service — Voca runs 24/7 and auto-restarts")
