...
async def emit_pipeline_event(node: str, status: str, **data) -> None:
    """Emit a pipeline node status event to the agent status SSE stream.

    Mirrors BaseAgent._broadcast_status() but for LangGraph pipeline nodes.
    """
    event = {"type": "pipeline", "node": node, "status": status, **data}
    try:
