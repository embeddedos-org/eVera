...
class PDFReadTool(Tool):
    def __init__(self):
        super().__init__(
            ...
            parameters={
                "file_path": {"type": "str", "description": "PDF path"},
                "pages": {"type": "str", "description": "Page range (e.g., 1-5 or all)"},
            },
        )
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        try:
            ...
            return {"status": "error", "message": "pip install PyPDF2"}
...
class PDFMergeTool(Tool):
    def __init__(self):
        super().__init__(
            ...
            parameters={
                "files": {"type": "str", "description": "Comma-separated PDF paths"},
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        try:
            ...
            for f in kw.get("files", "").split(","):
                merger.append(f.strip())
            out = kw.get("output", "merged.pdf")
            merger.write(out)
            merger.close()
...
class PDFSplitTool(Tool):
    def __init__(self):
        super().__init__(
            ...
            parameters={
                "file_path": {"type": "str", "description": "PDF path"},
                "pages": {"type": "str", "description": "Pages to extract (e.g., 1,3,5 or 1-5)"},
                "output": {"type": "str", "description": "Output path"},
            },
        )
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        try:
            ...
            if "-" in pages_str:
                s, e = map(int, pages_str.split("-"))
                page_nums = range(s - 1, e)
            else:
...
class PDFCreateTool(Tool):
    def __init__(self):
        super().__init__(
            name="pdf_create",
            description="Create PDF from text/HTML",
            parameters={
                "content": {"type": "str", "description": "Text or HTML content"},
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        try:
            ...
            out = kw.get("output", "document.pdf")
            c = canvas.Canvas(out, pagesize=letter)
            c.setTitle(kw.get("title", "Document"))
            c.setFont("Helvetica", 12)
            y = 750
            lines = kw.get("content", "").split("\n")
            for line in lines:
                if y < 50:
                    c.showPage()
                    c.setFont("Helvetica", 12)
            ...
            return {"status": "error", "message": "pip install reportlab"}
        except Exception as e:
            return {"status": "error", "message": str(e)}
...
class PDFInfoTool(Tool):
    def __init__(self):
        super().__init__(
            ...
            parameters={"file_path": {"type": "str", "description": "PDF path"}},
        )
...
class PDFAgent(BaseAgent):
    ...
    offline_responses = {
        "pdf": "\U0001f4c4 PDF ready!",
        "merge": "\U0001f4c4 Merging!",
        "split": "\u2702 Splitting!",
        "read": "\U0001f4d6 Reading!",
    }

    def _setup_tools(self):
        self._tools = [PDFReadTool(), PDFMergeTool(), PDFSplitTool(), PDFCreateTool(), PDFInfoTool()]
