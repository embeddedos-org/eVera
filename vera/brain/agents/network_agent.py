...
class PingTool(Tool):
    def __init__(self):
        super().__init__(
            ...
            parameters={
                ...
                "count": {"type": "int", "description": "Ping count"},
            },
        )
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        ...
        try:
            r = subprocess.run(
                ["ping", flag, str(kw.get("count", 4)), kw.get("host", "8.8.8.8")],
                capture_output=True,
                text=True,
                timeout=30,
            )
...
class TracerouteTool(Tool):
    def __init__(self):
        super().__init__(
            ...
            parameters={"host": {"type": "str", "description": "Target host"}},
        )
...
class SpeedTestTool(Tool):
    def __init__(self):
        super().__init__(name="speed_test", description="Test internet speed", parameters={})
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        try:
            ...
            return {"status": "error", "message": "pip install speedtest-cli"}
        except Exception as e:
            return {"status": "error", "message": str(e)}
...
class NetworkInfoTool(Tool):
    def __init__(self):
        super().__init__(name="network_info", description="Get network interface info", parameters={})
    ...
    async def execute(self, **kw: Any) -> dict[str, Any]:
        try:
            ...
            ifaces = {
                name: [{"address": a.address, "netmask": a.netmask} for a in addrs if a.address]
            ...
            return {
                "status": "success",
                "interfaces": ifaces,
                "io": {"sent_mb": round(stats.bytes_sent / 1e6, 2), "recv_mb": round(stats.bytes_recv / 1e6, 2)},
            }
        except ImportError:
...
class NetworkAgent(BaseAgent):
    ...
    system_prompt = (
        "You are eVera's Network Agent. Ping hosts, trace routes, test speed, get network info, WHOIS lookups."
    )
    offline_responses = {
        "ping": "\U0001f4e1 Pinging!",
        "speed": "\U0001f680 Speed test!",
        "network": "\U0001f310 Network!",
        "whois": "\U0001f50d Looking up!",
    }

    def _setup_tools(self):
        self._tools = [PingTool(), TracerouteTool(), SpeedTestTool(), NetworkInfoTool(), WhoisTool()]
