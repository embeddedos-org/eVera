...
Vera can receive commands from and send proactive notifications to messaging apps.
Each integration runs as a webhook listener on the FastAPI server.

Setup:
- Slack: Create a Slack App, set VERA_SLACK_BOT_TOKEN and VERA_SLACK_SIGNING_SECRET
- Discord: Create a Discord Bot, set VERA_DISCORD_BOT_TOKEN
- Telegram: Create via @BotFather, set VERA_TELEGRAM_BOT_TOKEN
"""
...
# Config from env
SLACK_BOT_TOKEN = os.getenv("VERA_SLACK_BOT_TOKEN", "")
SLACK_SIGNING_SECRET = os.getenv("VERA_SLACK_SIGNING_SECRET", "")
DISCORD_BOT_TOKEN = os.getenv("VERA_DISCORD_BOT_TOKEN", "")
TELEGRAM_BOT_TOKEN = os.getenv("VERA_TELEGRAM_BOT_TOKEN", "")
...
# ============================================================
# SLACK — Channel monitoring helpers
# ============================================================

_user_info_cache: dict[str, dict] = {}


async def fetch_channel_history(channel: str, oldest: str = "0", limit: int = 50) -> list[dict]:
    """Fetch recent messages from a Slack channel via conversations.history API."""
    if not SLACK_BOT_TOKEN:
        return []
    try:
        import httpx
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.warning("Slack conversations.history failed: %s", data.get("error", "unknown"))
    except Exception as e:
        logger.warning("fetch_channel_history failed: %s", e)
    return []


async def fetch_channel_info(channel: str) -> dict:
    """Fetch channel metadata via conversations.info API."""
    if not SLACK_BOT_TOKEN:
        return {}
    try:
        import httpx
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.warning("Slack conversations.info failed: %s", data.get("error", "unknown"))
    except Exception as e:
        logger.warning("fetch_channel_info failed: %s", e)
    return {}


async def get_user_info(user_id: str) -> dict:
    """Fetch Slack user info with module-level caching."""
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                user = data.get("user", {})
                _user_info_cache[user_id] = user
                return user
            logger.warning("Slack users.info failed: %s", data.get("error", "unknown"))
    except Exception as e:
    ...
    if text and chat_id:
        ...
        if text == "/start":
            await send_telegram_message(
                chat_id,
                "Hey! 👋 I'm Vera, your AI buddy! Just send me a message and I'll help you out!",
            )
            return {"status": "ok"}
